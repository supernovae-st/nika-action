...
class Honesty(unittest.TestCase):
    def test_unpriced_is_never_zero_dollars(self):
        body = rc.render(load("check-clean.json"), 0, "f.nika", "", "", "0.98.0")
...
class Findings(unittest.TestCase):
    def test_broken_shows_code_and_fails(self):
        body = rc.render(load("check-broken.json"), 2, "b.nika", "", "", "0.98.0")
        self.assertIn("❌", body)
...
class ParseFatal(unittest.TestCase):
    def test_non_json_renders_fenced_error(self):
        ...
        body = rc.render_parse_fatal(raw, 2, "p.nika", "0.98.0")
        self.assertIn("parse failed", body)
        self.assertIn("NIKA-PARSE-005", body)
        self.assertIn(rc.MARKER_FMT.format(path="p.nika"), body)
...
class Budget(unittest.TestCase):
    def test_verdict_survives_a_flooded_report(self):
        ...
        body = rc.render(report, 2, "b.nika", "", "", "0.98.0")
        self.assertLessEqual(len(body), 65_000)
        self.assertIn("nika check", body.splitlines()[0])
        self.assertIn(rc.MARKER_FMT.format(path="b.nika"), body)

    def test_marker_is_per_path_idempotence_key(self):
        a = rc.render(load("check-clean.json"), 0, "a.nika", "", "", "0.98.0")
        b = rc.render(load("check-clean.json"), 0, "b.nika", "", "", "0.98.0")
        self.assertIn("nika-action:v1:a.nika", a)
        self.assertIn("nika-action:v1:b.nika", b)


class Sections(unittest.TestCase):
    def test_mermaid_wraps_in_details(self):
        body = rc.render(load("check-clean.json"), 0, "f.nika",
                         "graph TD\n  a --> b", "", "0.98.0")
